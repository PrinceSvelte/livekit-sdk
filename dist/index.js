...
var reactCore = require('@livekit/react-core');
...
var AudioSelectButton = function AudioSelectButton(_ref) {
  ...
  var listAudioDevices = React.useCallback(function () {
    try {
      return Promise.resolve(livekitClient.Room.getLocalDevices('audioinput', requestPermissions)).then(function (devices) {
        setSources(devices);
...
var VideoSelectButton = function VideoSelectButton(_ref) {
  ...
  var listVideoDevices = React.useCallback(function () {
    try {
      return Promise.resolve(livekitClient.Room.getLocalDevices('videoinput', requestPermissions)).then(function (devices) {
        setSources(devices);
        setMenuItems(devices.map(function (item) {
          return {
            label: item.label
...
var ControlsView = function ControlsView(_ref) {
  ...
  var _useParticipant = reactCore.useParticipant(room.localParticipant),
...
var ParticipantView = function ParticipantView(_ref) {
  var participant = _ref.participant,
    ...
    onClick = _ref.onClick;
  var _useParticipant = reactCore.useParticipant(participant),
...
var ScreenShareView = function ScreenShareView(_ref) {
  ...
  }, React__default.createElement(reactCore.VideoRenderer, {
    track: track,
    isLocal: false,
    width: width,
    height: height,
...
var StageView = function StageView(stageProps) {
  ...
  return React__default.createElement("div", {
    ...
    return React__default.createElement(reactCore.AudioRenderer, {
      key: track.sid,
      track: track,
      isLocal: false
    });
...
var LiveKitRoom = function LiveKitRoom(_ref) {
  var url = _ref.url,
    ...
    onLeave = _ref.onLeave;
  var roomState = reactCore.useRoom(roomOptions);
  React.useEffect(function () {
    if (roomState.room) {
      roomState.connect(url, token, connectOptions).then(function (room) {
        ...
        if (onConnected && room.state === livekitClient.ConnectionState.Connected) {
          onConnected(room);
        }
      });
    }
    return function () {
      var _roomState$room;
      if (((_roomState$room = roomState.room) === null || _roomState$room === void 0 ? void 0 : _roomState$room.state) !== livekitClient.ConnectionState.Disconnected) {
