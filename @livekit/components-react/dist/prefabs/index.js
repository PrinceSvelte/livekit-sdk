...
function useMediaDeviceSelect({
  ...
  const deviceObserver = React9.useMemo(
    () => (0, import_components_core4.createMediaDeviceObserver)(kind, requestPermissions),
  ...
  const { className, activeDeviceObservable, setActiveMediaDevice } = React9.useMemo(
    () => (0, import_components_core4.setupDeviceSelector)(kind, room != null ? room : roomContext, track),
    [kind, room, roomContext, track]
  );
  React9.useEffect(() => {
    const listener = activeDeviceObservable.subscribe((deviceId) => {
      import_components_core4.log.info("setCurrentDeviceId", deviceId);
      if (deviceId)
...
function usePreviewTracks(options, onError) {
  ...
  React30.useEffect(() => {
    ...
    if (options.audio || options.video) {
      trackPromise = (0, import_livekit_client2.createLocalTracks)(options);
...
var PreJoin = (_a) => {
  ...
  const videoTrack = React30.useMemo(
    () => tracks == null ? void 0 : tracks.filter((track) => track.kind === import_livekit_client2.Track.Kind.Video)[0],
    [tracks]
  );
  const facingMode = React30.useMemo(() => {
    if (videoTrack) {
      const { facingMode: facingMode2 } = (0, import_livekit_client2.facingModeFromLocalTrack)(videoTrack);
      return facingMode2;
    } else {
      return "undefined";
    }
  ...
  const audioTrack = React30.useMemo(
    () => tracks == null ? void 0 : tracks.filter((track) => track.kind === import_livekit_client2.Track.Kind.Audio)[0],
    [tracks]
  );
  React30.useEffect(() => {
...
function LayoutContextProvider({
  ...
  React31.useEffect(() => {
    import_components_core9.log.debug("Widget Updated", { widgetState: layoutContextValue.widget.state });
...
function useTracks(sources = [
  import_livekit_client3.Track.Source.Camera,
  import_livekit_client3.Track.Source.Microphone,
  import_livekit_client3.Track.Source.ScreenShare,
  import_livekit_client3.Track.Source.ScreenShareAudio,
  import_livekit_client3.Track.Source.Unknown
], options = {}) {
  const room = useEnsureRoom(options.room);
...
function ControlBar(_a) {
  ...
  const browserSupportsScreenSharing = (0, import_components_core26.supportsScreenSharing)();
  const [isScreenShareEnabled, setIsScreenShareEnabled] = React52.useState(false);
  const onScreenShareChange = (enabled) => {
    setIsScreenShareEnabled(enabled);
  };
  return /* @__PURE__ */ React52.createElement("div", __spreadValues({ className: "lk-control-bar" }, props), visibleControls.microphone && /* @__PURE__ */ React52.createElement("div", { className: "lk-button-group" }, /* @__PURE__ */ React52.createElement(TrackToggle, { source: import_livekit_client8.Track.Source.Microphone, showIcon }, showText && "Microphone"), /* @__PURE__ */ React52.createElement("div", { className: "lk-button-group-menu" }, /* @__PURE__ */ React52.createElement(MediaDeviceMenu, { kind: "audioinput" }))), visibleControls.camera && /* @__PURE__ */ React52.createElement("div", { className: "lk-button-group" }, /* @__PURE__ */ React52.createElement(TrackToggle, { source: import_livekit_client8.Track.Source.Camera, showIcon }, showText && "Camera"), /* @__PURE__ */ React52.createElement("div", { className: "lk-button-group-menu" }, /* @__PURE__ */ React52.createElement(MediaDeviceMenu, { kind: "videoinput" }))), visibleControls.screenShare && browserSupportsScreenSharing && /* @__PURE__ */ React52.createElement(
    TrackToggle,
    {
...
function useConnectionQualityIndicator(options = {}) {
  ...
  const { className, connectionQualityObserver } = React53.useMemo(
    () => (0, import_components_core27.setupConnectionQualityIndicator)(p),
    [p]
  );
...
var useTrackMutedIndicator = (source, options = {}) => {
  ...
  const { className, mediaMutedObserver } = React55.useMemo(
    () => (0, import_components_core29.setupTrackMutedIndicator)(p, source),
    [p, source]
  );
...
function useFocusToggle({ trackSource, participant, props }) {
  const p = useEnsureParticipant(participant);
  const layoutContext = useMaybeLayoutContext();
  const { className } = React56.useMemo(() => (0, import_components_core30.setupFocusToggle)(), []);
  const inFocus = React56.useMemo(() => {
    ...
    if ((layoutContext == null ? void 0 : layoutContext.pin.state) && track) {
      return (0, import_components_core30.isTrackReferencePinned)(
...
function AudioVisualizer(_a) {
  ...
  const barCount = 7;
  const { track } = useMediaTrack(import_livekit_client12.Track.Source.Microphone, participant);
  React66.useEffect(() => {
...
function VideoConference(_a) {
  ...
  const tracks = useTracks(
    [
      { source: import_livekit_client14.Track.Source.Camera, withPlaceholder: true },
      { source: import_livekit_client14.Track.Source.ScreenShare, withPlaceholder: false }
    ],
    { updateOnlyOn: [import_livekit_client14.RoomEvent.ActiveSpeakersChanged] }
  );
  const widgetUpdate = (state) => {
    import_components_core35.log.debug("updating widget state", state);
    setWidgetState(state);
  };
  const layoutContext = useCreateLayoutContext();
  const screenShareTracks = tracks.filter(import_components_core35.isTrackReference).filter((track) => track.publication.source === import_livekit_client14.Track.Source.ScreenShare);
  const focusTrack = (_a2 = usePinnedTracks(layoutContext)) == null ? void 0 : _a2[0];
  const carouselTracks = tracks.filter((track) => !(0, import_components_core35.isEqualTrackRef)(track, focusTrack));
  React69.useEffect(() => {
    var _a3, _b3, _c, _d;
    if (screenShareTracks.length > 0 && lastAutoFocusedScreenShareTrack.current === null) {
      import_components_core35.log.debug("Auto set screen share focus:", { newScreenShareTrack: screenShareTracks[0] });
      ...
      import_components_core35.log.debug("Auto clearing screen share focus.");
  ...
  return /* @__PURE__ */ React69.createElement("div", __spreadValues({ className: "lk-video-conference" }, props), (0, import_components_core35.isWeb)() && /* @__PURE__ */ React69.createElement(
